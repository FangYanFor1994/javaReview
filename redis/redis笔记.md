...
```properties
1.什么是redis缓存雪崩？
#一句话概括：缓存中key在同一时间大面积失效，而同时有大量请求涌入，直接查询数据库，对数据库时灾难性的。
	目前电商首页以及热点数据都会做缓存，一般缓存都是定时任务去刷新，或者是在redis中查不到，从db中查到后更新到redis中的，用定时任务去刷新就会可能面临一个问题；
	举个例子：如果首页的key失效时间都是12个小时，中午12点刷新的，而零点有个秒杀活动大量用户涌入，此时缓存中的key都失效了。此时所有的请求全部落在数据库上，数据库必然扛不住可能会导致数据库直接挂掉。此时如果没有什么特别的方案处理这个故障，只是重启数据库，但是数据库立马又被新的流量打死了。
	
2.怎么处理缓存雪崩问题？
	1）在批量往redis存数据时，把每个key的失效时间都加一个随机值就好了，这样保证数据不会在同一时间大面积失效。
	2）或者设置热点数据永远不过期，有更新操作就更新到缓存就好了。
...
```properties
	1.什么是缓存穿透？
	#一句话概括：用户不断请求缓存和数据库中都没有的数据。
			比如数据库中id都是从1自增的，如果发起id值为-1的数据或id为特别大不存在的数据。这时的用户很可能是攻击者，攻击会导致数据库压力过大，严重时会击垮数据库。
	
2.解决缓存穿透：
	1）接口层加校验，如用户鉴权校验，id做基础校验，id<=0的直接拦截。
	2）从缓存中取不到数据，在数据库中也没有取到，这时可以将key-value对写为key-null，缓存有效时间可以设置短点，如30秒（设置太长会导致正常情况也没法使用）。这样可以防止攻击用户反复同一个id暴力攻击。
```
...
```properties
1.描述：
	缓存击穿是指缓存中没有但是数据库中有的数据（一般是缓存时间过期），这时由于并发用户特别多，同时读缓存没有读取到又同时从数据库取数据，引起数据库压力瞬间增大，造成过大压力。
	#注意：过多用户同时读缓存没有读到而又同时读数据库=====这句话的含义是并发量极大，如果不是并发量太大的话，一个用户从数据库中读取到数据然后写入到缓存，其他用户直接就可以从缓存中读取到数据了。就不会产生缓存击穿。
	
2.解决方案：
	1.设置热点数据永不过期。
	2.加互斥锁。保证同一时间只有一个请求可以访问数据库。
```

*注意*

1. 缓存击穿和缓存穿透概念混淆？ 缓存击穿指击穿redis缓存到达数据库，而缓存穿透指穿透redis缓存和数据库才叫穿透。
2. 雪崩和击穿区别？  雪崩是指key大面积失效，大部分的key都不可用从而都去数据库查询；    击穿指一个热点key过期，而大量请求同时请求数据库。